...
namespace ECSPhysics2D.Samples.FallingShapes
{
  ...
  public partial struct InputDetectionSystem : ISystem
  {
    ...
    {
      if (!SystemAPI.TryGetSingletonRW<ShapeSpawnerConfig>(out var config))
        return;
    ...
    private void SpawnRandomShape(ref SystemState state, ShapeSpawnerConfig config)
    {
    ...
    private void SpawnShapeAtPosition(ref SystemState state, float2 position, ShapeSpawnerConfig config)
    {
